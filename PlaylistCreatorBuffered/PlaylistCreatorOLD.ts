...
function helper_calculateDetune(rate: number) {
...
class SoundManager { //adapted from https://github.com/Vanilagy/mediabunny/blob/main/examples/media-player/media-player.ts (thank you!)
            ...
            try {
                ...
                while(true){
                    ...
                    createMediabunnyInternals: {
                        ...
                        await Promise.all([track.getNumberOfChannels(), track.getSampleRate()]).then((values: [number, number]) => {
    ...
    static shiftQueuedNodes(nextIndex: number | null = null){
        if(!isPlaying){
