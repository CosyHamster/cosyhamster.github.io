...
function resampleAudioBuffer(ctx, audioBuffer) {
    ...
    const offlineCtx = new OfflineAudioContext(audioBuffer.numberOfChannels, targetLength, ctx.sampleRate);
...
function resampleBuffer(buffer) {
    ...
    const offlineCtx = new OfflineAudioContext(buffer.numberOfChannels, numberOfFrames, ctx.sampleRate);
...
async function* resampledBufferIteratorOLD(bufferIterator, nChannels, inputSampleRate, currentID) {
