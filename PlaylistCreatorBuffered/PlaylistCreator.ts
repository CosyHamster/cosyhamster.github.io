...
class Deque{
    size = 0;
    front: { next?: any; prev?: any; value: any; } = undefined;
    back: { prev?: any; next?: any; value: any; } = undefined;
    constructor() {
...
class AudioDeque{
    size = 0;
    front: { next?: any; value: any; } = undefined;
    back: { next?: any; value: any; } = undefined;
    get length(){
        return this.size;
    }
    set length(_: number){
    ...
    push(value: AudioNode) {
        if(!this.front) this.front = this.back = { value };
        else this.back = this.back.next = { value };
        ++this.size;
    }
    *[Symbol.iterator](): Generator<AudioNode, void, AudioNode> {
        let ele = this.front;
        while(ele){
            yield ele.value;
            ele = ele.next;
...
var currentNode: AudioNode = null;
var scheduledNodes: AudioDeque = new AudioDeque();
var playRate = 1;
...
class AudioNode {
    ...
    constructor(node: AudioBufferSourceNode, timestamp: number, duration: number, startTimestamp: number) {
        this.node = node;
...
function helper_beginPlayingSoundIndex(index: number) {
...
function helper_seek(seconds: number) {
...
function replaceAudioContext(sampleRate: number) {
...
async function *resampledBufferIterator(bufferIterator: AsyncGenerator, nChannels: number, inputSampleRate: number, currentID: number){
    ...
    let src = null;
    try {
        let nextResult: IteratorResult<{buffer: AudioBuffer, timestamp: number, duration: number}, {buffer: AudioBuffer, timestamp: number, duration: number}> = (await bufferIterator.next());
...
class SoundManager { //adapted from https://github.com/Vanilagy/mediabunny/blob/main/examples/media-player/media-player.ts (thank you!)
    static playID: number = 0;
    static startTime: number = 0;
    static ctxStartTime: number = null;
    static ctxStartTimeDisplay: number = null;
    ...
    static startPlaying() {
        ...
        setIsBuffering(true);
        let bufferIterator: AsyncGenerator<unknown, any, unknown> = null;
            ...
            try {
                ...
                while(true){
                    ...
                    createMediabunnyInternals: {
                        ...
                        let success: boolean = await input.getPrimaryAudioTrack().then(async audioTrack => {
                            return Promise.all([audioTrack.getNumberOfChannels(), audioTrack.getSampleRate()]).then((values: [number, number]) => {
                                track = audioTrack;
    ...
    static shiftQueuedNodes(nextSong: Song | null = null){
        if(!isPlaying){
...
let codecs: { [x: string]: boolean; mp3: boolean; mpeg: boolean; opus: boolean; ogg: boolean; oga: boolean; wav: boolean; aac: boolean; caf: boolean; m4a: boolean; m4b: boolean; mp4: boolean; weba: boolean; webm: boolean; dolby: boolean; flac: boolean; aif: boolean; aiff: boolean; aff: boolean; };
...
function canPlay(extension: string): boolean {
...
var MAIN_TABLE = document.body.querySelector(".mainTable") as HTMLTableElement,
    PLAYLIST_VIEWER_TABLE = document.getElementById("Playlist_Viewer") as HTMLTableElement,
    BUFFER_SECONDS = document.getElementById('bufferSeconds') as HTMLInputElement,
    ...
    MOBILE_CONTEXT_BUTTONS = document.getElementById("mobileContextButtons") as HTMLDivElement,
...
function progressBarSeek(mouse: PointerEvent, hoverType: ProgressBarSeekAction): void {
    if (currentSongIndex === null || (mouse?.pointerType == "touch" && hoverType !== ProgressBarSeekAction.SEEK_TO) || hoverType === ProgressBarSeekAction.STOP_DISPLAYING){
    ...
    const duration: number = sounds[currentSongIndex].duration;
    if(duration === null){
        HOVERED_TIME_DISPLAY.style.transform = "translate(-9999px, 0px)";
        return;
    }
...
async function addFiles(files: ArrayLike<File> /*FileList or File[]*/) {
...
function setIsBuffering(buffering: boolean){
...
//ts-ignore
function shuffle(b: number,c: number,d: Song){//array,placeholder,placeholder,placeholder
    c=sounds.length;
    while(c){
        b = Math.random()*c--|0;
        d = sounds[c];
...
function onClickSpecificPlaySong(checkbox: HTMLInputElement){
    const song: Song = sounds[tryFindTableRowInParents(checkbox).rowIndex-1];
    startOrUnloadSong(song, checkbox.checked);
}

function startOrUnloadSong(song: Song, startPlaying: boolean){
    if(startPlaying) startPlayingSpecificSong(song);
...
function startPlayingSpecificSong(song: Song){ //called by HTML element
    changeStatus(StatusTexts.BUFFERING);
...
function isValidExtension(extension: string) { return canPlay(extension); }
//@ts-ignore
...
var activeRow: HTMLTableRowElement|null = null;
function setActiveRow(row: HTMLTableRowElement|null){
    const previouslyActiveRow = activeRow;
    activeRow = row;

    if(previouslyActiveRow && previouslyActiveRow !== row){
        (previouslyActiveRow.firstElementChild.querySelector("input.playpause") as HTMLInputElement).checked = false;
        updateRowColor(previouslyActiveRow); //activeRow.style.backgroundColor = RowColors.NONE;
    }

    if(row){
        (row.firstElementChild.querySelector("input.playpause") as HTMLInputElement).checked = true;
        updateRowColor(row);
    }
}
...
function onSingleClick(mouseEvent: MouseEvent | PointerEvent) {
    ...
    if(mouseEvent instanceof curWin.PointerEvent && (mouseEvent as PointerEvent).pointerType != "mouse") {
...
function arrowSelection(keyboardEvent: KeyboardEvent, indexIncrement: number) {
    ...
    sortSelectedRows();
    if (!keyboardCanInteract(keyboardEvent)) return;
...
function deleteSongsFromKeyboard(keyboardEvent: KeyboardEvent) { if (keyboardCanInteract(keyboardEvent)) deleteSelectedSongs(); }
function startPlayingFromKeyboard(keyboardEvent: KeyboardEvent) {
    if(!keyboardCanInteract(keyboardEvent) || selectedRows.length !== 1) return;
    keyboardEvent.preventDefault();
...
function keyboardCanInteract(keyEvent: KeyboardEvent): boolean {
    const target = keyEvent.target;
    return !(target instanceof curWin.HTMLInputElement) && (target as HTMLElement).closest("dialog") === null;
}
...
function onPlayListRightClick(mouseEvent: MouseEvent | PointerEvent) {
...
function spawnRowContextMenu(clientX: number, clientY: number, showDefaultOptions: boolean){
    const contextOptions: ContextMenuOption[] = getPlaylistContextOptions();
