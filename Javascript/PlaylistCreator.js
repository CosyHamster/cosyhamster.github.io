...
class DataTransferItemGrabber { //this exists because javascript has bugs
  static PhaseTypes = Object.freeze({
    COLLECTING: 0,
    RETRIEVING: 1,
    FINISHED: 2
  });
  ...
  filesCollected = 0;
  filesAdded = 0;
  phase = DataTransferItemGrabber.PhaseTypes.COLLECTING; //0 == collecting, 1 == retrieving
  ...
  async retrieveContents() {
    return new Promise(async resolve => {
      ...
      this.phase = DataTransferItemGrabber.PhaseTypes.RETRIEVING;
      await Promise.allSettled(this.promises);
      this.phase = DataTransferItemGrabber.PhaseTypes.FINISHED;
  ...
  updateLoadingStatus() {
    switch (this.phase) {
      case DataTransferItemGrabber.PhaseTypes.COLLECTING: return changeStatus(`Collecting: (${this.filesCollected} files; ${this.filesAdded} processed)`);
      case DataTransferItemGrabber.PhaseTypes.RETRIEVING: return changeStatus(`Processed: ${this.filesAdded}/${this.filesCollected} files`);
      case DataTransferItemGrabber.PhaseTypes.FINISHED: return changeStatus(`Adding ${this.filesAdded} to the playlist... (this will lag)`)
    }
  }
}
//December 2, 2023, 2:39AM
var REQUEST_ANIMATION_FRAME_EVENT = new OnRequestAnimationFrameEvent(),
  KEY_DOWN_EVENT = new OnKeyDownEvent(),
  VALAD_FILE_EXTENSIONS = new Set(["ogg", "webm", "wav", "hls", "flac", "mp3", "opus", "pcm", "vorbis", "aac"]),
  ProgressBarSeekActions = {
    SEEK_TO: 0,
    DISPLAY_TIME: 1,
    STOP_DISPLAYING: 2
  },
  ...
  PAUSED = false,
  PLAYING = true,
  PLAYLIST_VIEWER_TABLE = document.getElementById("Playlist_Viewer"),
  PRELOAD_DIST_ELEMENT = document.getElementById('preloadDistance'),
  COMPACT_MODE_LINK_ELEMENT = null,//document.getElementById('compactModeStyleLink'),
  COMPACT_MODE_TOGGLE = document.getElementById('compactMode'),
  SEEK_DURATION_NUMBER_INPUT = document.getElementById('seekDuration'),
  SEEK_DURATION_DISPLAY = document.getElementById("seekDurationDisplay"),
  SEEK_DISTANCE_PROPORTIONAL_CHECKBOX = document.getElementById('seekDistanceProportional'),
  SKIP_UNPLAYABLE_CHECKBOX = document.getElementById('skipUnplayable'),
  UPLOAD_BUTTON = document.getElementById('0input'),
  UPLOAD_DIRECTORY_BUTTON = document.getElementById('inputDirectory'),
  PLAY_RATE_RANGE = document.getElementById('0playRateSlider'),
  SETTINGS_PAGE = document.getElementById('settingsPage'),
  ERROR_POPUP = document.getElementById('errorPopup'),
  ERROR_LIST/*: HTMLDListElement*/ = document.getElementById('errorList'),
  PROGRESS_BAR = document.getElementById('progress-bar'),
  HOVERED_TIME_DISPLAY = document.getElementById('hoveredTimeDisplay'),
  VOLUME_CHANGER = document.getElementById('0playVolume'),
  PLAY_RATE = document.getElementById('0playRate'),
  PLAY_PAN = document.getElementById('0playPan'),
  SEEK_BACK = document.getElementById('seekBack'),
  SEEK_FORWARD = document.getElementById('seekForward'),
  REPEAT_BUTTON = document.getElementById('repeatButton'),
  SHUFFLE_BUTTON = document.getElementById('shuffleButton'),
  MUTE_BUTTON = document.getElementById('0Mute'),
  PLAY_BUTTON = document.getElementById('playpause'),
  STATUS_TEXT = document.getElementById('0status'),
  CURRENT_FILE_NAME = document.getElementById('currentFileName'),
  DROPPING_FILE_OVERLAY = document.getElementById("dragOverDisplay"),
  DURATION_OF_SONG_DISPLAY = document.getElementById('secondDurationLabel');
...
const start = (() => {
  ...
  PROGRESS_BAR.addEventListener('pointerenter', (pointer) => progressBarSeek(pointer, ProgressBarSeekActions.DISPLAY_TIME), { passive: true })
  PROGRESS_BAR.addEventListener('pointerdown', (pointer) => { if(pointer.button == 0) progressBarSeek(pointer, ProgressBarSeekActions.SEEK_TO); }, { passive: true })
  PROGRESS_BAR.addEventListener('pointermove', (pointer) => progressBarSeek(pointer, ProgressBarSeekActions.DISPLAY_TIME), { passive: true })
  PROGRESS_BAR.addEventListener('pointerleave', (pointer) => progressBarSeek(pointer, ProgressBarSeekActions.STOP_DISPLAYING), { passive: true })
  //END
})()
...
function makeDocumentDroppable() {
  ...
  window.addEventListener("drop", (event) => {
    ...
    if (!onlyFiles(dataTransferObject)) return;
    event.preventDefault();
    DROPPING_FILE_OVERLAY.setAttribute("draggingOver", false);
    stopHighlightingRow();
    importFiles(dataTransferObject);
  });
}
function onlyFiles(dataTransferObject) { return dataTransferObject.types.length == 1 && dataTransferObject.types[0] == 'Files' }
...
function progressBarSeek(mouse, hoverType) {
  if ((mouse?.pointerType == "touch" && hoverType !== ProgressBarSeekActions.SEEK_TO) || sounds[currentSongIndex] == null || sounds[currentSongIndex]?.state?.() != 'loaded' || hoverType === ProgressBarSeekActions.STOP_DISPLAYING) return HOVERED_TIME_DISPLAY.setAttribute('inUse', 0);

  const offsetX = mouse.offsetX,
    progressBarWidth = PROGRESS_BAR.clientWidth,
    currentSongLength = sounds[currentSongIndex].duration();

  let seekToTime = Math.max(new Number(offsetX * (currentSongLength / progressBarWidth)), 0);
  switch (hoverType) {
    case (ProgressBarSeekActions.SEEK_TO): return sounds[currentSongIndex].seek(seekToTime);
    case (ProgressBarSeekActions.DISPLAY_TIME):
...
/**
 * @param errorType The name of the exception
 * @param errorText Generic error message to explain the error better.
 * @param errorMessage The message provided by the error
*/
