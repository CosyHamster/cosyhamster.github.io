...
`:"")+`#EXT-X-ENDLIST
`:"");this.format._options.onPlaylist?.(u,To(e));let l=await this.output._getTarget({path:c,isRoot:!1,mimeType:Et}),m=new Bt(l,!0);m.start(),m.write($.encode(u)),await m.flush(),await m.finalize()}async writeMasterPlaylist(){p(this.output._target instanceof Ne);let e=this.output._target,r=`#EXTM3U
